...
/**
 * BLOCAGES — l'équivalent des KEY BLOCKERS de Miles.
 *
 * La carte la plus utile de son OS, et celle qui manquait le plus au tien :
 * elle ne montre pas ce qu'il faut faire, mais **ce qui est arrêté et qui
 * attend quelqu'un**. Le nombre de jours bloqués est ce qui pique — une
 * relance oubliée depuis 19 jours devient impossible à ignorer.
 */

const CHALEUR: Record<Blocage["chaleur"], { label: string; couleur: string }> = {
  chaud: { label: "CHAUD", couleur: "var(--color-mag)" },
  tiede: { label: "TIÈDE", couleur: "var(--color-amb)" },
  froid: { label: "FROID", couleur: "var(--color-ble)" },
};
  ...
  return (
    <Panel accent="var(--color-mag)" className="col-span-1">
      <div className="flex items-center justify-between gap-2">
        ...
        >
          {blocages.length} actif{blocages.length > 1 ? "s" : ""}
        </span>
      </div>

      {blocages.length === 0 ? (
        <EmptyState hint="Tout avance.">Rien de bloqué</EmptyState>
      ) : (
        <>
          <div className="mt-3 flex flex-col gap-[7px]">
            {visibles.map((b) => {
              ...
              return (
                ...
                >
                  <div className="text-[12px] font-bold leading-[1.3]">{b.texte}</div>
